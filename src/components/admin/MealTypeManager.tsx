  ...
  return (
    <section className="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 transition-colors">
      <div className="flex justify-between items-start gap-4 mb-1">
        <h2 className="text-lg font-medium text-gray-900 dark:text-gray-100">{t('mealTypes')}</h2>
        ...
        <table className="w-full text-left border-collapse text-sm">
          ...
          <tbody>
            {mealTypes.map((type, index) => {
              const draft = draftOf(type);
              return (
                <tr key={type.id} className={`border-b border-gray-100 dark:border-gray-700 last:border-0 hover:bg-gray-50/50 dark:hover:bg-gray-700/30 transition-colors ${type.active ? '' : 'opacity-50'}`}>
                  <td className="p-2 whitespace-nowrap">
                    ...
                    <button onClick={() => move(index, 1)} disabled={index === mealTypes.length - 1} className="px-1 text-gray-500 dark:text-gray-400 disabled:opacity-30" aria-label="down">▼</button>
                  </td>
