...
export default function PaymentsInbox({ users, managerUid, monthPayments }: Props) {
  ...
  const inputClass = 'w-full px-3 py-2 bg-gray-50 dark:bg-gray-700 border border-gray-200 dark:border-gray-600 text-gray-900 dark:text-gray-100 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/20 transition-colors';

  return (
    <section className="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 space-y-6 transition-colors">
      <div>
        <h2 className="text-lg font-medium text-gray-900 dark:text-gray-100 mb-1">{t('payments')}</h2>
        <p className="text-sm text-gray-500 dark:text-gray-400">{t('paymentsHint')}</p>
      </div>

      <div>
        <h3 className="text-sm font-semibold text-gray-700 dark:text-gray-300 mb-2">
          {t('pendingPayments')} {pending.length > 0 && <span className="ml-1 bg-amber-100 dark:bg-amber-950/60 text-amber-800 dark:text-amber-300 text-xs px-2 py-0.5 rounded-full">{pending.length}</span>}
        </h3>
        {pending.length === 0 ? (
          <p className="text-sm text-gray-400 dark:text-gray-500">{t('noPendingPayments')}</p>
        ) : (
          <ul className="divide-y divide-gray-100 dark:divide-gray-700 border border-gray-100 dark:border-gray-700 rounded-lg">
            {pending.map((p) => (
              <li key={p.id} className="flex flex-col sm:flex-row sm:items-center gap-2 px-4 py-3">
                ...
                <div className="flex gap-2">
                  ...
                  <button onClick={() => setStatus(p, 'rejected')} disabled={busy === p.id} className="text-xs font-medium text-red-600 dark:text-red-400 border border-red-200 dark:border-red-800 px-3 py-1.5 rounded-lg hover:bg-red-50 dark:hover:bg-red-950/40 disabled:opacity-50">{t('reject')}</button>
                </div>
              </li>
            ))}
          </ul>
      ...
      <form onSubmit={record} className="bg-gray-50 dark:bg-gray-700/50 border border-gray-100 dark:border-gray-700 rounded-lg p-4 grid grid-cols-2 md:grid-cols-6 gap-3 items-end transition-colors">
        <div className="col-span-2 md:col-span-6"><h3 className="text-sm font-semibold text-gray-700 dark:text-gray-300">{t('recordPayment')}</h3></div>
        <div className="col-span-2 md:col-span-2">
          <label className="block text-xs text-gray-500 dark:text-gray-400 mb-1">{t('member')}</label>
        ...
        <div>
          <label className="block text-xs text-gray-500 dark:text-gray-400 mb-1">{t('purpose')}</label>
        ...
        <div className="col-span-2 md:col-span-2">
          <label className="block text-xs text-gray-500 dark:text-gray-400 mb-1">{t('amountTk')}</label>
          <AmountInput value={form.amount} onChange={(v) => setForm({ ...form, amount: v })} chips={[500, 1000, 2000]} className="!py-2 !text-sm" />
        </div>
        <div className="col-span-2 md:col-span-4">
          <label className="block text-xs text-gray-500 dark:text-gray-400 mb-1">{t('note')} ({t('optional')})</label>
      ...
      {confirmedThisMonth.length > 0 && (
        <div>
          <h3 className="text-sm font-semibold text-gray-700 dark:text-gray-300 mb-2">{t('thisMonthPayments')}</h3>
            ...
            <table className="w-full text-sm border-collapse">
              ...
              <tbody>
                {confirmedThisMonth.map((p) => (
                  <tr key={p.id} className="border-b border-gray-100 dark:border-gray-700 last:border-0 hover:bg-gray-50/50 dark:hover:bg-gray-700/30 transition-colors">
                    <td className="p-2 text-gray-500 dark:text-gray-400">{formatDateId(p.data.date)}</td>
                    <td className="p-2 text-gray-900 dark:text-gray-100">{nameOf(p.data.user_id)}</td>
                    <td className="p-2 text-gray-500 dark:text-gray-400">{t(PURPOSE_KEYS[p.data.purpose] ?? 'purposeFund')}</td>
                    <td className="p-2 text-right text-gray-900 dark:text-gray-100 font-medium">{formatTk(p.data.amount, lang, 0)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
