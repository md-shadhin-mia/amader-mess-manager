  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-950 pb-12">
      <PageHeader title={t('monthReports')} backTo="/admin">
        <Link to="/admin/reports" className="text-sm font-medium text-gray-600 dark:text-gray-300 border border-gray-200 dark:border-gray-700 px-3 py-1.5 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-800">{t('yearOverview')}</Link>
      </PageHeader>
      <main className="max-w-4xl mx-auto p-6 space-y-6">
        <section className="bg-white dark:bg-gray-900 rounded-xl shadow-sm border border-gray-100 dark:border-gray-800 p-4 flex flex-col sm:flex-row sm:items-end gap-3">
          <div className="flex-1">
            <h2 className="text-sm font-semibold text-gray-700 dark:text-gray-200">{t('addPastMonth')}</h2>
            <p className="text-xs text-gray-500 dark:text-gray-400">{t('addPastMonthHint')}</p>
          </div>
          <select value={pastMonth} onChange={(e) => setPastMonth(e.target.value)} className="px-3 py-2 bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 text-gray-900 dark:text-gray-100 rounded-lg text-sm">
            <option value="">—</option>
        ...
        <section className="bg-white dark:bg-gray-900 rounded-xl shadow-sm border border-gray-100 dark:border-gray-800 overflow-hidden">
          {loading ? (
            ...
            <p className="p-6 text-sm text-gray-400 dark:text-gray-500">{t('noMonths')}</p>
          ) : (
              ...
              <tbody>
                {months.map((m) => (
                  <tr key={m.id} className="border-b border-gray-100 dark:border-gray-800 last:border-0 hover:bg-gray-50 dark:hover:bg-gray-800/40">
                    <td className="p-4 font-medium text-gray-900 dark:text-white">{formatMonthId(m.id)}</td>
                    <td className="p-4"><span className={`px-2 py-1 rounded text-xs font-medium ${statusBadge(m.status)}`}>{statusLabel(m.status)}</span></td>
                    <td className="p-4 text-right text-gray-700 dark:text-gray-300">{m.status === 'closed' ? formatCount(m.total_meals ?? 0, lang) : '—'}</td>
                    <td className="p-4 text-right text-gray-700 dark:text-gray-300">{m.status === 'closed' ? formatTk(m.meal_rate ?? 0, lang) : '—'}</td>
                    <td className="p-4 text-right text-gray-700 dark:text-gray-300">{m.grand ? formatTk(m.grand.net_payable, lang, 0) : '—'}</td>
                    <td className="p-4 text-right whitespace-nowrap">
                      {isMonthEditable(m) && <Link to={`/admin/months/${m.id}/edit`} className="text-blue-600 dark:text-blue-400 hover:text-blue-800 dark:hover:text-blue-300 font-medium mr-3">{t('editEntries')}</Link>}
                      <Link to={`/admin/months/${m.id}`} className="text-blue-600 dark:text-blue-400 hover:text-blue-800 dark:hover:text-blue-300 font-medium">{t('viewReport')} →</Link>
                    </td>
                  </tr>
                ))}
              </tbody>
