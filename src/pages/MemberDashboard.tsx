    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900 pb-12 transition-colors">
      ...
      <header className="bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 px-6 py-4 flex flex-col md:flex-row justify-between items-start md:items-center gap-4 sticky top-0 z-10 transition-colors">
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <Link to="/member/months" className="text-sm font-medium text-blue-600 dark:text-blue-400 hover:underline">{t('myMonth')}</Link>
          <button onClick={() => setShowProfile(true)} className="text-sm font-medium text-blue-600 dark:text-blue-400 hover:underline">{t('editProfile')}</button>
          <ThemeToggle />
      ...
      <main className="max-w-4xl mx-auto p-6 space-y-6">
        ...
        <section className="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 transition-colors">
          <div className="flex justify-between items-center mb-4">
            <h2 className="text-lg font-medium text-gray-900 dark:text-gray-100">{t('thisMonth')} {activeMonth && <span className="text-gray-400 dark:text-gray-500 font-normal">· {formatMonthId(activeMonth.id)}</span>}</h2>
            {activeMonth && activeMonth.status === 'active' && <Link to={`/member/months/${activeMonth.id}`} className="text-sm font-medium text-blue-600 dark:text-blue-400 hover:underline">{t('breakdown')} →</Link>}
          </div>
          {!activeMonth ? (
            <p className="text-sm text-gray-500 dark:text-gray-400">{t('noActiveMonth')}</p>
          ) : (
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              <div className="bg-gray-50 dark:bg-gray-700/50 p-4 rounded-lg">
                <p className="text-sm text-gray-500 dark:text-gray-400 mb-1">{t('totalMeals')}</p>
                <p className="text-2xl font-semibold text-gray-900 dark:text-gray-100">{formatCount(row?.meal_count ?? 0, lang)}</p>
                {totals && <p className="text-xs text-gray-400 dark:text-gray-500">{t('mealRate')} {formatTk(totals.meal_rate, lang)}</p>}
              </div>
              <div className="bg-gray-50 dark:bg-gray-700/50 p-4 rounded-lg">
                <p className="text-sm text-gray-500 dark:text-gray-400 mb-1">{t('dueNow')}</p>
                <p className={`text-2xl font-semibold ${(row?.due_now ?? 0) > 0 ? 'text-red-700 dark:text-red-400' : 'text-green-700 dark:text-green-400'}`}>{formatTk(row?.due_now ?? 0, lang, 0)}</p>
              </div>
              <div className="bg-gray-50 dark:bg-gray-700/50 p-4 rounded-lg">
                <p className="text-sm text-gray-500 dark:text-gray-400 mb-1">{t('estimatedAtClose')}</p>
                <p className="text-2xl font-semibold text-gray-900 dark:text-gray-100">{formatTk(row?.net_payable ?? 0, lang, 0)}</p>
              </div>
              <div className="bg-gray-50 dark:bg-gray-700/50 p-4 rounded-lg">
                <p className="text-sm text-gray-500 dark:text-gray-400 mb-1">{t('deposits')}</p>
                <p className="text-2xl font-semibold text-gray-900 dark:text-gray-100">{formatTk(row?.credits.deposits ?? 0, lang, 0)}</p>
                {pendingTotal > 0 && <p className="text-xs text-amber-700 dark:text-amber-400">{t('statusPending')}: {formatTk(pendingTotal, lang, 0)}</p>}
              </div>
            </div>
          )}
        </section>
        ...
        <section className="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 transition-colors">
          <h2 className="text-lg font-medium text-gray-900 dark:text-gray-100 mb-4">{t('ledgerSummary')}</h2>
          <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
            <div className="bg-gray-50 dark:bg-gray-700/50 p-4 rounded-lg">
              <p className="text-sm text-gray-500 dark:text-gray-400 mb-1">{t('myRoomRent')}</p>
              <p className="text-2xl font-semibold text-gray-900 dark:text-gray-100">{typeof userProfile?.room_rent === 'number' ? formatTk(userProfile.room_rent, lang, 0) : t('notSet')}</p>
            </div>
            <div className="bg-gray-50 dark:bg-gray-700/50 p-4 rounded-lg">
              <p className="text-sm text-gray-500 dark:text-gray-400 mb-1">{t('advanceBalance')}</p>
              <p className="text-2xl font-semibold text-gray-900 dark:text-gray-100">{formatTk(userProfile?.advance_balance ?? 0, lang, 0)}</p>
            </div>
            <div className="bg-gray-50 dark:bg-gray-700/50 p-4 rounded-lg">
              <p className="text-sm text-gray-500 dark:text-gray-400 mb-1">{t('personalBazar')}</p>
              <p className="text-2xl font-semibold text-gray-900 dark:text-gray-100">{formatTk(row?.credits.personal_bazar ?? 0, lang, 0)}</p>
            </div>
          </div>
        </section>
