...
function Line({ label, value, strong, tone }: { label: string; value: string; strong?: boolean; tone?: 'red' | 'green' }) {
  return (
    <div className={`flex justify-between py-2 border-b border-gray-100 dark:border-gray-800 last:border-0 ${strong ? 'font-semibold' : ''}`}>
      <span className="text-gray-600 dark:text-gray-400">{label}</span>
      <span className={tone === 'red' ? 'text-red-700 dark:text-red-400' : tone === 'green' ? 'text-green-700 dark:text-green-400' : 'text-gray-900 dark:text-white'}>{value}</span>
    </div>
  );
}
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-950 pb-12">
      <PageHeader title={t('myMonth')} subtitle={userProfile?.name} backTo={backTo}>
        <select
          ...
          className="text-sm px-3 py-1.5 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 text-gray-900 dark:text-gray-100 rounded-lg"
      ...
      <main className="max-w-2xl mx-auto p-4 md:p-6 space-y-4">
        ...
        {!loading && !month && <p className="text-sm text-gray-500 dark:text-gray-400">{t('noMonths')}</p>}
        {!loading && month && mode === 'legacy' && <p className="text-sm text-amber-800 dark:text-amber-200 bg-amber-50 dark:bg-amber-950/40 border border-amber-200 dark:border-amber-800 rounded-lg p-3">{t('legacyMonthHint')}</p>}
        {!loading && month && mode !== 'legacy' && !row && <p className="text-sm text-gray-500 dark:text-gray-400">{t('noSettlementRow')}</p>}

        {month && row && totals && (
          <>
            <div className="flex items-center gap-2 text-sm">
              <span className={`px-2 py-1 rounded font-medium ${mode === 'live' ? 'bg-green-100 dark:bg-green-900/30 text-green-800 dark:text-green-300' : 'bg-gray-200 dark:bg-gray-800 text-gray-800 dark:text-gray-200'}`}>
                {mode === 'live' ? t('preview') : t('closed')}
              </span>
              <span className="text-gray-500 dark:text-gray-400">{t('mealRate')}: {formatTk(totals.meal_rate, lang)}</span>
            </div>

            <section className="grid grid-cols-2 gap-3">
              <div className="bg-white dark:bg-gray-900 rounded-xl border border-gray-100 dark:border-gray-800 shadow-sm p-4">
                <p className="text-xs text-gray-500 dark:text-gray-400">{mode === 'live' ? t('dueNow') : t('netPayable')}</p>
                <p className={`text-2xl font-semibold ${row.net_payable > 0 ? 'text-red-700 dark:text-red-400' : 'text-green-700 dark:text-green-400'}`}>
                  {formatTk(mode === 'live' ? row.due_now : row.net_payable, lang, 0)}
                </p>
              </div>
              <div className="bg-white dark:bg-gray-900 rounded-xl border border-gray-100 dark:border-gray-800 shadow-sm p-4">
                <p className="text-xs text-gray-500 dark:text-gray-400">{mode === 'live' ? t('estimatedAtClose') : t('advanceAfter')}</p>
                <p className="text-2xl font-semibold text-gray-900 dark:text-white">{formatTk(mode === 'live' ? row.net_payable : row.advance_after, lang, 0)}</p>
              </div>
            </section>

            <section className="bg-white dark:bg-gray-900 rounded-xl border border-gray-100 dark:border-gray-800 shadow-sm p-4">
              <h2 className="text-sm font-semibold text-gray-700 dark:text-gray-200 mb-2">{t('mealsByType')}</h2>
              <div className="flex flex-wrap gap-2 mb-2">
                {Object.entries(row.meals).map(([typeId, n]) => {
                  const type = mealTypes.find((m) => m.id === typeId);
                  return (
                    <span key={typeId} className="px-3 py-1 rounded-full bg-gray-100 dark:bg-gray-800 text-sm text-gray-700 dark:text-gray-300">
                      {type ? labelOf(type, lang) : typeId}: <strong>{formatCount(n, lang)}</strong>
                    </span>
                  );
                })}
                {Object.keys(row.meals).length === 0 && <span className="text-sm text-gray-400 dark:text-gray-500">{t('noEntriesYet')}</span>}
              </div>
            ...
            <section className="bg-white dark:bg-gray-900 rounded-xl border border-gray-100 dark:border-gray-800 shadow-sm p-4">
              <h2 className="text-sm font-semibold text-gray-700 dark:text-gray-200 mb-2">{t('breakdown')}</h2>
              <p className="text-xs text-gray-400 dark:text-gray-500 mb-1">{t('postpaidSection')}</p>
              <Line label={t('mealCost')} value={formatTk(row.meal_cost, lang, 0)} />
              {postpaid.map((c) => <Line key={c.id} label={labelOf(c, lang)} value={formatTk(row.shares[c.id] ?? 0, lang, 0)} />)}
              {prepaid.length > 0 && <p className="text-xs text-gray-400 dark:text-gray-500 mt-3 mb-1">{t('prepaidSection')}</p>}
            ...
            <section className="bg-white dark:bg-gray-900 rounded-xl border border-gray-100 dark:border-gray-800 shadow-sm p-4">
              <h2 className="text-sm font-semibold text-gray-700 dark:text-gray-200 mb-2">{t('credits')}</h2>
